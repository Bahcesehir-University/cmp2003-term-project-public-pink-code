...
bool TripAnalyzer::SlotKeyEq::operator()(const std::pair<std::string, int>& x, const std::pair<std::string, int>& y) const noexcept
{
...
std::vector<ZoneCount> TripAnalyzer::topZones(int k) const
{
    ...
    for (const auto& e : zoneCounts_) res.push_back({ e.first, e.second });
...
std::vector<SlotCount> TripAnalyzer::topBusySlots(int k) const
{
    ...
    for (const auto& e : slotCounts_) res.push_back({ e.first.first, e.first.second, e.second });
